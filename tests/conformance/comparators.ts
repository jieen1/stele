...
function compareViolation(
  ...
  // Round 4 D-12 + Round 5 J-13: `cause.summary` accepts either exact
  // equality OR substring match. The substring relaxation lets fixtures
  // declare just the discriminating fragment instead of pinning entire
  ...
  if (expected.cause.summary.trim().length === 0) {
    ...
    actual.cause.summary !== expected.cause.summary &&
