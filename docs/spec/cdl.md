...
**Incremental source-change skipping (`--changed-from <ref>` / `--changed <files...>`).** This is an inner-loop speedup, distinct from `--diff`/`--diff-from` (which filter *invariants* by changed *contract* files). `--changed-from <ref>` collects the changed *source* files (branch diff vs `<ref>`, unioned with staged/unstaged/untracked, fail-closed if git is unavailable); `--changed` takes an explicit file list. The check then **skips only the `code-shape` stage** (boundary / class-shape / function-shape / type-policy / file-policy) when its reverse-index covered files — computed by the same `expandContractToFiles` primitive that powers `stele coverage` — are disjoint from the changed set. Every other stage always runs.

Why only `code-shape`: a stage is sound to skip *only* when the files it examines provably equal its reverse-index file set. That holds for the code-shape family — each declaration inspects exactly the AST of its own target file(s). It does **not** hold for the call-graph or whole-universe mechanisms, whose examination surface is non-local, so they are **never** skipped:

- `trace`: extern targets exist only as call-graph *edges* (not nodes, so they expand to an empty file set), and path constraints (`must-transit`/`deny-transit`/…) examine intermediate nodes anywhere in the graph — neither is captured by the scope-caller files.
- `effect`: effect sets *propagate* up from callees, so editing an out-of-scope callee changes an in-scope node's effective effects.
- `type-state`, `architecture`, `core-node`: call-graph / whole-universe module maps with non-local examination.

This is conservative by design — correctness over speed. The whole-repo / whole-contract stages (`generated`, `protected`/manifest, `design`, `toolchain`, `type-driven`, and the pytest checker suite) also always run. A `[incremental]` banner on stderr lists exactly which stages ran and which were skipped, so the output never silently implies full coverage. A full `stele check` (no `--changed*`) remains authoritative and is what CI runs.
